...
(function () {
  ...
  let pendingDuplicateData = null; // { duplicates, nonDuplicates, source, totalIncoming }

  window.addEventListener('duplicates-found', (e) => {
    pendingDuplicateData = e.detail;
    showDuplicateModal(pendingDuplicateData);
  });
  ...
  function showDuplicateModal(data) {
    ...
    const { duplicates, nonDuplicates, totalIncoming } = data;

    // Count how many existing records would get a category upgrade
    const upgrades = duplicates.filter(d => !d.keepExisting);

    let summaryText = `Found <strong>${duplicates.length}</strong> duplicate${duplicates.length > 1 ? 's' : ''} out of ${totalIncoming} incoming transactions. `;
    summaryText += `<strong>${nonDuplicates.length}</strong> new transaction${nonDuplicates.length !== 1 ? 's' : ''} will be imported.`;
    if (upgrades.length > 0) {
      summaryText += `<br><span style="color:var(--clr-primary)">📋 ${upgrades.length} existing transaction${upgrades.length > 1 ? 's' : ''} will have ${upgrades.length > 1 ? 'their categories' : 'its category'} updated from the incoming data.</span>`;
    }
    summary.innerHTML = summaryText;

    // Render duplicate table
    let html = '<table><thead><tr><th></th><th>Date</th><th>Description</th><th>Amount</th><th>Category</th><th>Status</th></tr></thead><tbody>';
    for (const dup of duplicates) {
      const ex = dup.existing;
      const inc = dup.incoming;
      const keepExBadge = dup.keepExisting
        ? '<span class="dup-keep-badge keep">✓ Keep</span>'
        : '<span class="dup-keep-badge keep">✓ Keep</span><span class="dup-category-upgraded">category updated</span>';
      const discardBadge = dup.keepExisting
        ? '<span class="dup-keep-badge discard">✕ Discard</span>'
        : '<span class="dup-keep-badge discard">✕ Discard</span><span class="dup-category-upgraded">better category</span>';

      html += `<tr class="dup-row-existing">
        <td>Existing</td>
        <td>${ex.date}</td>
        <td>${escHtml(ex.description)}</td>
        <td>$${Math.abs(ex.amount).toFixed(2)}</td>
        <td>${ex.category}</td>
        <td>${keepExBadge}</td>
      </tr>`;
      html += `<tr class="dup-row-incoming" style="border-bottom: 2px solid var(--clr-border)">
        <td>Incoming</td>
        <td>${inc.date}</td>
        <td>${escHtml(inc.description)}</td>
        <td>$${Math.abs(inc.amount).toFixed(2)}</td>
        <td>${inc.category || 'Other'}</td>
        <td>${discardBadge}</td>
      </tr>`;
    }
    html += '</tbody></table>';
    ...
    if (duplicates.length > 20) {
      html = `<p style="font-size:.8rem;color:var(--clr-text-muted);margin-bottom:.5rem">Showing all ${duplicates.length} duplicate pairs:</p>` + html;
    }
  ...
  document.getElementById('btn-dup-cancel').addEventListener('click', () => {
    if (pendingDuplicateData) {
      const source = pendingDuplicateData.source;
      Importer.finishImport(source, 'Import cancelled — duplicates detected.', '');
    }
    hideDuplicateModal();
  });
