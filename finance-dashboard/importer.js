...
const Importer = (() => {
  ...
  function acceptPdfTransactions() {
    if (pendingPdfTransactions.length > 0) {
      console.log(`[Importer] Checking ${pendingPdfTransactions.length} transactions for duplicates`);
      const { duplicates, nonDuplicates } = DataManager.findDuplicates(pendingPdfTransactions);

      if (duplicates.length > 0) {
        // Dispatch event to show duplicate modal — let app.js handle the UI
        window.dispatchEvent(new CustomEvent('duplicates-found', {
          detail: {
            duplicates,
            nonDuplicates,
            source: 'pdf',
            totalIncoming: pendingPdfTransactions.length
          }
        }));
        return; // Don't import yet — modal will handle it
      }
  ...
  function importCSVData() {
    ...
    const { duplicates: csvDuplicates, nonDuplicates: csvNonDuplicates } = DataManager.findDuplicates(transactions);

    if (csvDuplicates.length > 0) {
      window.dispatchEvent(new CustomEvent('duplicates-found', {
        detail: {
          duplicates: csvDuplicates,
          nonDuplicates: csvNonDuplicates,
          source: 'csv',
  ...
  function acceptJsonTransactions() {
    if (pendingJsonTransactions.length > 0) {
      const { duplicates, nonDuplicates } = DataManager.findDuplicates(pendingJsonTransactions);
      ...
      if (duplicates.length > 0) {
        window.dispatchEvent(new CustomEvent('duplicates-found', {
          detail: {
            ...
            nonDuplicates,
            source: 'json',
            totalIncoming: pendingJsonTransactions.length
          }
        }));
        return;
      }
  ...
  return {
    ...
    finishImport(source, message, statusClass = 'success') {
      if (source === 'pdf') {
        ...
        document.getElementById('pdf-status').className = 'status-text ' + statusClass;
      } else if (source === 'json') {
        pendingJsonTransactions = [];
        document.getElementById('json-preview-area').classList.add('hidden');
        document.getElementById('json-status').textContent = message;
        document.getElementById('json-status').className = 'status-text ' + statusClass;
      } else if (source === 'csv') {
        csvRawData = null;
        document.getElementById('csv-mapping-area').classList.add('hidden');
        document.getElementById('csv-status').textContent = message;
        document.getElementById('csv-status').className = 'status-text ' + statusClass;
      }
    }
  };
})();
